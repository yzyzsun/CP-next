...
merge (a:[StateInfo]) (b:[StateInfo]) : [StateInfo] = 
  ...
  else if (head @StateInfo a).population < (head @StateInfo b).population then ([head @StateInfo a] ++ (merge (tail @StateInfo a) b))
...
doc T = trait [self : DocSig<T> & CiteSig<T> & TableSig<T>] => {
  ...
  body = `\Section[Smallest Microstates By Population]\\`;
};
